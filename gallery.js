...
setTimeout(() => {  //only one time and setInterval repetdly
  if (db) {
    let imageDBTRansaction = db.transaction("image", "readonly"); //Creating a transction arg 1.objetstore 2.types of transction
    ...
    imageRequest.onsuccess = () => {
      ...
      imageResult.forEach((imageObj) => {
        ...
        let url = imageObj.url; // made url form object
    ...
    videoRequest.onsuccess = () => {
      ...
      videoResult.forEach((videoObj) => {
        ...
        let url = URL.createObjectURL(videoObj.blobData);//in img and video main differnce is here that the link from obect of video is made by blob
...
function deleteListener(e) {
  ...
  let type = id.split("-")[0]; //- befor this 0th element it take
  console.log(type);
  if (type == "vid") {
